...
int g_tsap_st_dianjiao_enable_num = 2;
...
void tsap_st_dianjiao_process_t(void *dummy)
{
    ...
    for(;;)
    {
        if( g_tsap_st_dianjiao_new_cmd )        // 有新命令要处理
        {
            ...
            for( i = 0; i < g_tsap_st_dianjiao_enable_num; i++ )
            {
                for(;;)
                {
                    TSAP_sys_get_status(&sys_status, NULL, NULL);
                    if( sys_status != 6 )
                    {
                        break;
                    }
            ...
            while (1)
            {
                for (i = 0; i < g_tsap_st_dianjiao_enable_num; i++)
                {
                    reached = tsap_stn_check_reached(dianjiao_st_en_id[i]);
                    if (FALSE == reached)
                    {
                        break;
                    }
                ...
                //如果全部要求的工位都到达了，就完成退出，否则stn_reached_num归零再查过
                if (stn_reached_num == g_tsap_st_dianjiao_enable_num)
                {
                    for (i = 0; i < g_tsap_st_dianjiao_enable_num; i++)
                    {
                        dianjiao_st_en_id[i] = 0xff;//用完恢复0xff
                    }
                    break;
                }
                else
       ...
       )
{
   ...
   for(i=0;i<g_max_tsap_dianjiao_st_num;i++)
   {
        ...
        {
            return rtn;//dianjiao工位有动子，说明上一轮还没有完成，这一轮不做
        }
   }
    if( !g_tsap_st_dianjiao_process )       // 点胶流程线程空闲中，这次命令才去处理，否则丢弃
    {
        ...
        g_tsap_st_dianjiao_enable_num = num;
    ...
    for(;;)
    {
        if( g_tsap_st_zuzhuang_new_cmd )        // 有新命令要处理
        {
            ...
            while (1)
            {
                for (i = 0; i < g_tsap_st_zuzhuang_cmd_num; i++)
                {
                    reached = TSAP_st_get_mover_id(zuzhuang_st_en_id[i], &reached, &mover_id);//tsap_stn_check_reached(zuzhuang_st_en_id[i]);
                    if (TRUE == reached)
                    {
                        for(j=0;j<g_tsap_st_zuzhuang_cmd_num;j++)//有g_tsap_st_zuzhuang_cmd_num个动子放过来了
                        {
                            if(mover_id==bufferd_mover_id[j])
                            {
                                stn_left--;
       ...
       )
{
    ...
    for(i=0;i<g_max_tsap_zhuzhuang_st_num;i++)
   {
        if(tsap_stn_check_reached(buffer_st_id+i+1))
        {
            return rtn;//zuzhuang工位有动子，说明上一轮还没有完成，这一轮不做
        }
   }
    if( !g_tsap_st_zuzhuang_process)       // 组装流程线程空闲中，这次命令才去处理，否则丢弃
    {
        // 找到enable的工位列表
        for( i = 0; (i<st_num && i<8); i++ )
        {
