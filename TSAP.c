...
int g_TSAP_flush_sys_errinfo_rtn = 0;
int  g_TSAP_flush_stn_status = 0;

// 是否启动工位使能、禁止检测保护  
BOOL g_tsap_stn_enable_flush = FALSE;
BOOL g_tsap_stn_disable_flush = FALSE;

// 系统中最大的工位id
int g_station_id_max = 34;   

int g_station_num = 35;     // 工位个数

int g_max_tsap_dianjiao_st_num = 3;
int g_tsap_dianjiao_buffer_st_id = 6;
int g_max_tsap_zhuzhuang_st_num = 4;
int g_tsap_zhuzhuang_buffer_st_id = 11;
...
void tsap_st_dianjiao_process_t(void *dummy);
void tsap_st_zuzhuang_process_t(void *dummy);
...
int eth_read_config_2_var(IN char *name, IN char *value)
{
    ...
    else if( strstr(name, "station_num") != NULL )  // 工位个数
    {
        g_station_num = atoi(value);
        g_station_id_max = g_station_num - 1;
    }
    else if( strstr(name, "stn_enable_flush_EN") != NULL )  // 工位个数
    {
        g_tsap_stn_enable_flush = atoi(value);
    }
    else if( strstr(name, "stn_disable_flush_EN") != NULL )  // 工位个数
    {
        g_tsap_stn_disable_flush = atoi(value);
    }

    else if( strstr(name, "dianjiao_st_num") != NULL )     // 点胶工位个数
    {
        g_max_tsap_dianjiao_st_num= atoi(value);
    }
    else if( strstr(name, "dianjiao_buffer_st_id") != NULL )     // 点胶缓冲工位id
    {
        g_tsap_dianjiao_buffer_st_id = atoi(value);
    }
    else if( strstr(name, "zhuzhuang_st_num") != NULL )     // 组装工位个数
    {
        g_max_tsap_zhuzhuang_st_num= atoi(value);
    }
    else if( strstr(name, "zhuzhuang_buffer_st_id") != NULL )     // 组装缓冲工位id
    {
        g_tsap_zhuzhuang_buffer_st_id = atoi(value);
    }
    else
    {
    }
        ...
        )
{
	...
	BOOL save_to_flash = TRUE;
    char *func_name = "TSAP_st_set_station_para";
    int link_err[2] = { 0 };
...
#if 0
    if( id > g_station_id_max )
    {
        EEXA_log_error(rtn, link_err, __FILE__, __LINE__,
            "warning: %s(st_id: %d) input id > station_id_max", func_name, id);
    }
        ...
        )
{
    ...
    if( station_id > g_station_id_max )
    {
        EEXA_log_error(rtn, link_err, __FILE__, __LINE__,
            "warning: %s(st_id: %d) input id > station_id_max", func_name, station_id);
    }
...
//its_udp_lock();
    ...
    {
        // get stn_reached 
        value_0 = g_mio_stn_status_reg.stn_reached_0;
        value_1 = g_mio_stn_status_reg.stn_reached_32;
        if( st_id > 31 )
        {
            value = value_1;
            mover_reached = tsxa_is_bit_set(value, st_id - 32);
        }
        else
        {
            value = value_0;
            mover_reached = tsxa_is_bit_set(value, st_id);
        }
        ...
        )
{
    ...
    int  target_pos = 0;
    double error = 0.1;
    char *func_name = "TSAP_axis_move_abs";

    UINT16 all_data[6] = { 0 };

    DTXA_trace_t("TS", DTXA_TRACE_DEBUG, func_name, "> (axis = %d pos = %g vel = %g acc = %g dec = %g )", axis, pos, vel, acc, dec);
...
//    rtn = MIO_write_reg_shorts(addr, all_data, 6);
    ...
    DTXA_trace_t("TS", DTXA_TRACE_DEBUG, func_name, "< () = 0x%x", rtn);
        ...
        )
{
    ...
    double cur_vel = 0.0;
    UINT16 all_data[6] = { 0 };
    char *func_name = "TSAP_axis_limit_move_abs";

    DTXA_trace_t("TS", DTXA_TRACE_DEBUG, func_name, "> (axis = %d pos = %g max_offset = %g vel = %g )", axis, pos, max_offset, vel);
        ...
        )
{
    ...
    // 获取是否到达
    reached = g_mio_stn_status_reg.stn_reached_0;
    for( i = 0; i < 32; i++ )
    {
        st_axis_reached[i] = tsxa_is_bit_set(reached, i);
    }
        ...
        )
{
    ...
    // 获取是否到达
    reached = g_mio_stn_status_reg.stn_reached_32;
    for( i = 0; i < 32; i++ )
    {
        st_axis_reached[i] = tsxa_is_bit_set(reached, i);
    }
        ...
        )
{
    ...
    int addr_read = 0;
    int addr_write = 0;
    char *func_name = "TSAP_st_set_station_pv";
    int link_err[2] = { 0 };
...
void tsap_flush_mio_status_reg_t(void *dummy)
{
    int rtn = OK;
    UINT32 data[2] = { 0 };
    ...
    for(;;)
    {
        ...
        if( !g_network_initialized )
        {
//            break;    // 去除，否则如果初始化时网络有问题，就会导致线程退出了
        }
...
void tsap_flush_mio_errinfo_reg_t(void *dummy)
{
    int rtn = OK;
    UINT32 data[2] = { 0 };
    UINT32 value = 0;
    ...
    for(;;)
    {
        g_TSAP_flush_sys_errinfo_rtn = MIO_read_reg(0x102c, &value);
        if( OK == g_TSAP_flush_sys_errinfo_rtn )
        {
...
// 禁用35号以上不用的工位，使能34及以下使用的工位   for test.
void tsap_disable_high_station_t(void *dummy)
{
    int rtn = OK;
    int st_id = 0;
    int axis = 0;
    ...
    for(;;)
    {
        ...
        if( g_tsap_stn_enable_flush )
        {
            for(i = 0; i < g_station_num; i++ )
            {
                rtn = TSAP_st_enable(i);
                Sleep(10);
            }
        }

        if( g_tsap_stn_disable_flush )
        {
            for(i = g_station_num; i < 64; i++ )
            {
                rtn = TSAP_st_disable(i);
                Sleep(10);
            }
        }
...
int tsap_disable_high_station(void)
{
    int rtn = OK;

    _beginthread(tsap_disable_high_station_t, 8*1024*1024, NULL);

    return rtn;
}

void tsap_flush_mio_stn_reg_t(void *dummy)
{
    int rtn = OK;
    UINT32 stn_data[32] = { 0 };
...
int tsap_start_read_mio_reg_thread(void)
{
    ...
    _beginthread(tsap_flush_mio_status_reg_t, 16*1024*1024, NULL);
    Sleep(200);

    _beginthread(tsap_flush_mio_errinfo_reg_t, 16*1024*1024, NULL);
    Sleep(200);

    _beginthread(tsap_flush_mio_stn_reg_t, 16*1024*1024, NULL);
    Sleep(200);

    // stn en/disable check list
    tsap_disable_high_station();
    Sleep(200);

    // 启动点胶工位处理线程
    _beginthread(tsap_st_dianjiao_process_t, 16*1024*1024, NULL);
    Sleep(200);
    
	// 启动组装工位处理线程
    _beginthread(tsap_st_zuzhuang_process_t, 16*1024*1024, NULL);
    Sleep(200);
    return rtn;
}


BOOL tsap_stn_check_reached(int st_id)
{
    UINT32 value_0;
    UINT32 value_1;
    UINT32 value;
    ...
    return mover_reached;
}

// 下发的允许运行的点胶工位个数，每次命令下发进来可能会修改
int g_tsap_st_dianjiao_cmd_num = 2;

BOOL g_tsap_st_dianjiao_process = FALSE;   // 点胶流程是否处理中
BOOL g_tsap_st_dianjiao_new_cmd = FALSE;   // 新的点胶流程命令是否下发了

int dianjiao_st_en_id[8] = { 0xff };       // enable的工位列表
...
void tsap_st_dianjiao_process_t(void *dummy)
{
    ...
    int i = 0;
    int j = 0;
    UINT32 sys_status = 0, sys_errno = 0,  sys_errinfo = 0;
    BOOL reached = 0;
    int mover_id;
    int stn_left = 0;
    int bufferd_mover_id[8]={0xfc, 0xfc, 0xfc, 0xfc, 0xfc, 0xfc, 0xfc, 0xfc};
    int buffered_mover_num = 0;

    for(;;)
    {
        if( g_tsap_st_dianjiao_new_cmd )        // 有新命令要处理
        {
            g_tsap_st_dianjiao_new_cmd = FALSE;
            g_tsap_st_dianjiao_process = TRUE;      // 流程处理中，暂时不接新命令
                
            stn_left = g_tsap_st_dianjiao_cmd_num;
            buffered_mover_num = 0;

            while(stn_left>0)//还有目标工位没有到达，轮询执行放行动子和查询到达的工作
            {
                if(buffered_mover_num<g_tsap_st_dianjiao_cmd_num)
                {//还有动子没有放过去
                    // 发送缓冲工位上的动子
                    reached = FALSE;
                    rtn = TSAP_st_get_mover_id(g_tsap_dianjiao_buffer_st_id, &reached, &mover_id);
                    if( OK == rtn )
                    {
                        if( reached )
                        {
                            TSAP_st_move_next_station(g_tsap_dianjiao_buffer_st_id);
                            bufferd_mover_id[buffered_mover_num]=mover_id;
                            buffered_mover_num++;
                        }
                    }
                }

                for (i = 0; i < g_tsap_st_dianjiao_cmd_num; i++)
                {//查询所有目标工位上是否有动子到达
                    TSAP_st_get_mover_id(dianjiao_st_en_id[i], &reached, &mover_id);//tsap_stn_check_reached(zuzhuang_st_en_id[i]);
                    if (TRUE == reached)
                    {
                        for(j=0;j<buffered_mover_num;j++)//有buffered_mover_num个动子放过来了
                        {//查询到达的动子是否是这一轮放过来的动子
                            if((0xfc!=mover_id) && (mover_id==bufferd_mover_id[j]))
                            {
                                stn_left--;
                                bufferd_mover_id[j]=0xfc;//清除记录的动子id
                                break;
                            }
                        }
                    }
                ...
                rtn = TSAP_sys_get_status(&sys_status, &sys_errno, &sys_errinfo);

                if( (sys_status!=6) && (sys_status!=0xE))//0xE暂停和0x6都算是在autowork
                {
                    for(j=0;j<buffered_mover_num;j++)
                    {//这一轮提前退出了，清除记录的动子id
                        bufferd_mover_id[j]=0xfc;
                    }
                    break;
                }
                Sleep(20);
            ...
            g_tsap_st_dianjiao_process = FALSE;
        }
        else
        {
            Sleep(20);
...
// 用于点胶工位的工位选择 
// 大致流程:
// 前面有1个缓冲工位，下发这个命令给出来有效的工位列表，就将前面缓冲区工位发车几个动子到点胶工位去
// 动子都到达以后，完成退出，否则就一直等待完成，不接受新的发车命令
int  TSAP_st_dianjiao_set
        (
        IN int st_num,
       ...
       )
{
    ...
    int num = 0;
    int i = 0;
    int dianjiao_st_id= g_tsap_dianjiao_buffer_st_id+1;     // 点胶工位列表，认为buffer工位之后就是组装工位
    int dianjiao_st_num = 3;                                // 点胶工位数量
    int buffer_st_id = 6;        // 缓冲工位id, 从配置文件读取
    BOOL reached = FALSE;
    int mover_id = 0;
    BOOL buffer_st_reached = FALSE;
    int buffer_moved_num = 0;
    UINT32 sys_status = 0;

   dianjiao_st_num = g_max_tsap_dianjiao_st_num;
   buffer_st_id = g_tsap_dianjiao_buffer_st_id;

   for(i=0;i<g_max_tsap_dianjiao_st_num;i++)
   {
        if(tsap_stn_check_reached(buffer_st_id+i+1))
        {
            return 1;//dianjiao工位有动子，说明上一轮还没有完成，这一轮不做
        }
   }
    if( !g_tsap_st_dianjiao_process )       // 点胶流程线程空闲中，这次命令才去处理，否则丢弃
    {
        // 找到enable的工位列表
        for( i = 0; (i<st_num && i<8); i++ )
        {
            if( st_list[i] != 0 )
            {
                dianjiao_st_en_id[i] = st_list[i];
                num = num + 1;
            }
        }

        g_tsap_st_dianjiao_cmd_num = num;

        // 先把所有点胶工位disable
        for( i = 0; i < dianjiao_st_num; i++ )
		{
			TSAP_st_disable(dianjiao_st_id);
			dianjiao_st_id++;
 	   }
        
        Sleep(80);
        
        // 再将需要enable的station enable
        for( i = 0; i < num; i++ )
        {
            TSAP_st_enable(dianjiao_st_en_id[i]);
        }
        Sleep(80);
        
        g_tsap_st_dianjiao_new_cmd = TRUE;   // 通知线程有新命令要处理
    }
    else
    {
        printf("st_dianjiao in process, do nothing, quit...\n");
    }    
    
    return rtn;
}

// 下发的允许运行的组装工位个数，每次命令下发进来可能会修改
int g_tsap_st_zuzhuang_cmd_num = 2;

BOOL g_tsap_st_zuzhuang_process = FALSE;   // 点胶流程是否处理中
BOOL g_tsap_st_zuzhuang_new_cmd = FALSE;   // 新的点胶流程命令是否下发了

int zuzhuang_st_en_id[8] = { 0xff };       // enable的工位列表
BOOL g_all_fcm_mover_given = FALSE;//为了诊断
BOOL g_all_fcm_mover_rched = FALSE;
// 组装流程处理线程
void tsap_st_zuzhuang_process_t(void* dummy)
{
    int rtn = OK;
    ...
    for(;;)
    {
        if( g_tsap_st_zuzhuang_new_cmd )        // 有新命令要处理
        {
            g_all_fcm_mover_given = FALSE;
            g_all_fcm_mover_rched = FALSE;
            g_tsap_st_zuzhuang_new_cmd = FALSE;
            g_tsap_st_zuzhuang_process = TRUE;      // 流程处理中，暂时不接新命令
            stn_left = g_tsap_st_zuzhuang_cmd_num;
            buffered_mover_num = 0;

            while(stn_left>0)//还有目标工位没有到达，轮询执行放行动子和查询到达的工作
            {
                if(buffered_mover_num<g_tsap_st_zuzhuang_cmd_num)
                {//还有动子没有放过去
                    // 发送缓冲工位上的动子
                    reached = FALSE;
                    rtn = TSAP_st_get_mover_id(g_tsap_zhuzhuang_buffer_st_id, &reached, &mover_id);
                    if( OK == rtn )
                    {
                        if( reached )
                        {
                            TSAP_st_move_next_station(g_tsap_zhuzhuang_buffer_st_id);
                            bufferd_mover_id[buffered_mover_num]=mover_id;
                            buffered_mover_num++;
                        }
                    }
                ...
                for (i = 0; i < g_tsap_st_zuzhuang_cmd_num; i++)
                {//查询所有目标工位上是否有动子到达
                    TSAP_st_get_mover_id(zuzhuang_st_en_id[i], &reached, &mover_id);//tsap_stn_check_reached(zuzhuang_st_en_id[i]);
                    if (TRUE == reached)
                    {
                        for(j=0;j<buffered_mover_num;j++)//有buffered_mover_num个动子放过来了
                        {//查询到达的动子是否是这一轮放过来的动子
                ...
                if(buffered_mover_num==g_tsap_st_zuzhuang_cmd_num)
                {
                    g_all_fcm_mover_given = TRUE;
                }
                Sleep(20);
            }

            g_all_fcm_mover_rched = TRUE;
            
            g_tsap_st_zuzhuang_process = FALSE;
        }
        else
        {
            Sleep(20);
...
// rtn: 0 OK; 1 fcm工位有动子，不执行;
//      2 还有动子没有放行
//      3 还有动子没有到达工位
// 用于组装工位的工位选择
int  TSAP_st_zhuzhuang_set
        (
        IN int st_num,
       ...
       )
{
    ...
    int num = 0;
    int i = 0;
    int zuzhuang_st_id=g_tsap_zhuzhuang_buffer_st_id+1;     // 组装工位，认为buffer工位之后就是组装工位
    int zuzhuang_st_num = 4;                                // 组装工位数量
    int buffer_st_id = 11;        // 缓冲工位id, 从配置文件读取
    BOOL reached = FALSE;
    int mover_id = 0;
    BOOL buffer_st_reached = FALSE;
    int buffer_moved_num = 0;
    ...
    zuzhuang_st_num = g_max_tsap_zhuzhuang_st_num;
    buffer_st_id = g_tsap_zhuzhuang_buffer_st_id;

    for(i=0;i<g_max_tsap_zhuzhuang_st_num;i++)
   {
        if(tsap_stn_check_reached(buffer_st_id+i+1))
        {
            return 1;//zuzhuang工位有动子，说明上一轮还没有完成，这一轮不做
        }
   }
    if( 0==g_tsap_st_zuzhuang_process)       // 组装流程线程空闲中，这次命令才去处理，否则丢弃
    {
        // 找到enable的工位列表
        for( i = 0; (i<st_num && i<8); i++ )
        {
            if( st_list[i] != 0 )
            {
                zuzhuang_st_en_id[i] = st_list[i];
                num = num + 1;
            }
        }

        g_tsap_st_zuzhuang_cmd_num = num;
        // 先把所有组装工位disable
        for( i = 0; i < zuzhuang_st_num; i++ )
        {
            TSAP_st_disable(zuzhuang_st_id);
		    zuzhuang_st_id++;
        }
        ...
        for( i = 0; i < num; i++ )
        {
            TSAP_st_enable(zuzhuang_st_en_id[i]);
        }

        Sleep(80);

        g_tsap_st_zuzhuang_new_cmd = TRUE;   // 通知线程有新命令要处理

    }
    else
    {
        if(!g_all_fcm_mover_rched)
        {
            rtn = 3;
        }
        
        if(!g_all_fcm_mover_given)
        {
            rtn = 2;
        }
        
        printf("st_zuzhuang in process, do nothing, quit...\n");
    }    
